...
1.  Lean toward more plain language when explaining code. The user is familiar with R, but not a regular programmer and has little experience developing full tools or programs. Walk through a new concept step-by-step and explain tool choices/reasoning.
...
-   **Code Organization:** Always start new scripts with a header that includes title, description (1-3 sentences), and final output(s) (what the main function returns). Use section dividers and put any configs in the first section, helper functions in the next section, and the main function in a new section.
...
5.  **Configuration:** Any notes about package or AI model configuration; can also point to a `config/README.md` if used.
...
-   **Commit Message Format:** use past tense describing what was changed; focus on what and why, not how; keep subject line concise but comprehensive.
    ...
    -   Example: "Update \_\_\_\_.R to solve API connection issues + Refactor to use tidyverse"
...
## Project Context (to be edited for each project)

### Overview

Goal: A tool that calls an AI model to assist with creating an event planning worksheet based on an existing template

Output: A .csv or Microsoft Excel sheet

Intended Users: Nontechnical individual contributors who are operating as project support and planning large events such as convenings.
